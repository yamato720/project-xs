...
$(TARGET): $(GOLDEN_MAIN_SRC) $(PORT_SRC) $(PORT_GROUP_SRC) $(AXI_SRC) $(ERROR_SRC) $(SIM_SRC) $(SESSION_SRC) $(KERNEL_SRC) $(KERNEL_COMPONENT_SRC) $(GOLDEN_SOLVER_SRC) $(GOLDEN_DATASET_SRC) $(BASE_INCLUDE_DIR)/CycleSimulator.h $(BASE_INCLUDE_DIR)/SimulationSession.h $(BASE_INCLUDE_DIR)/Kernel.h $(BASE_INCLUDE_DIR)/KernelComponent.h $(BASE_INCLUDE_DIR)/Port.h $(BASE_INCLUDE_DIR)/PortGroup.h $(BASE_INCLUDE_DIR)/Axi.h $(BASE_INCLUDE_DIR)/Error.h | $(BUILD_DIR)
	$(CXX) $(CXXFLAGS) -I$(SRC_DIR) -I$(INCLUDE_DIR) $(GOLDEN_MAIN_SRC) $(PORT_SRC) $(PORT_GROUP_SRC) $(AXI_SRC) $(ERROR_SRC) $(SIM_SRC) $(SESSION_SRC) $(KERNEL_SRC) $(KERNEL_COMPONENT_SRC) -o $(TARGET)
